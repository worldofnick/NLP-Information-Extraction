...
def classify(article, common_weapons, killing_verbs, orgs, en_nlp):
    ...
    targets = detect_targets(article.cased_text, killing_verbs, en_nlp)
    victims = detect_victims(article.cased_text)
    individuals = detect_individuals(article.cased_text, killing_verbs, en_nlp)

    return ExtractedInfo(article.id, incident_type, weapons, individuals, orgs, targets, victims)
